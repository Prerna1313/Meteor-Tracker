...
const createAsteroidDust = (): THREE.Points => {
...
const createMeteors = (): THREE.Group => {
...
const createKuiperMeteors = (): THREE.Group => {
...
export function SolarSystem({
  data, selectedObjectId, onSelectObject, hoveredObjectId, onHoverObject,
}: SolarSystemProps) {
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      const handleControlsStart = (event: any) => {
    ...
    const animate = () => {
      ...
      stateRef.celestialObjects.forEach((objGroup, id) => {
        ...
        if (objData.type === 'planet' || objData.type === 'comet') {
          ...
          const x_ecl = x_orb * (Math.cos(argOfPeri) * Math.cos(Omega) - Math.sin(argOfPeri) * Math.sin(Omega) * Math.cos(i))
            - y_orb * (Math.sin(argOfPeri) * Math.cos(Omega) + Math.cos(argOfPeri) * Math.sin(Omega) * Math.cos(i));
          const z_ecl = x_orb * (Math.cos(argOfPeri) * Math.sin(Omega) + Math.sin(argOfPeri) * Math.cos(Omega) * Math.cos(i))
            + y_orb * (Math.cos(argOfPeri) * Math.cos(Omega) * Math.cos(i) - Math.sin(argOfPeri) * Math.sin(Omega));
    ...
    screenLabels
      ...
      .forEach((label) => {
        ...
        for (const existingLabel of nonOverlappingLabels) {
          if (
            Math.sqrt(
              Math.pow(label.screenX - existingLabel.screenX, 2) + Math.pow(label.screenY - existingLabel.screenY, 2)
            ) < labelSpacing
          ) {
            overlaps = true;
            break;
...
return (
  <div ref={mountRef} className="absolute top-0 left-0 w-full h-full z-10" style={{ touchAction: 'none' }}>
    {/* The overlay covers only the map area, NOT header */}
        ...
        return (
          <div
            ...
            className={`absolute flex items-center gap-1.5 p-1 rounded-sm transition-all duration-300 pointer-events-auto cursor-pointer uppercase tracking-wider text-xs hover:font-bold ${
              ...
              ((hoveredObjectId && hoveredObjectId !== label.id) ||
              (selectedObjectId && selectedObjectId !== label.id)
                ? ' opacity-50'
                : ' opacity-100')
            }`}
            style={{
